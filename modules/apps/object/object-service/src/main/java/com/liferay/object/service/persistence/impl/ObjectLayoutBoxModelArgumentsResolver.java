...
/**
 * The arguments resolver class for retrieving value from ObjectLayoutBox.
 *
 * @author Marco Leo
 * @generated
 */
@Component(
	immediate = true,
	service = {
		ObjectLayoutBoxModelArgumentsResolver.class, ArgumentsResolver.class
	}
)
public class ObjectLayoutBoxModelArgumentsResolver
	...
	public Object[] getArguments(
		...
		if (!checkColumn || (columnBitmask == 0)) {
			return _getValue(objectLayoutBoxModelImpl, columnNames, original);
		}
	...
	private static Object[] _getValue(
		ObjectLayoutBoxModelImpl objectLayoutBoxModelImpl, String[] columnNames,
		boolean original) {
		...
		for (int i = 0; i < arguments.length; i++) {
			...
			if (original) {
				arguments[i] = objectLayoutBoxModelImpl.getColumnOriginalValue(
					columnName);
			}
			else {
