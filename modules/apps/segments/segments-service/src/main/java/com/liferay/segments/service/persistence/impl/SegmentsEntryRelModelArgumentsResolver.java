...
/**
 * The arguments resolver class for retrieving value from SegmentsEntryRel.
 *
...
@Component(
	immediate = true,
	service = {
		SegmentsEntryRelModelArgumentsResolver.class, ArgumentsResolver.class
	}
)
public class SegmentsEntryRelModelArgumentsResolver
	...
	public Object[] getArguments(
		...
		if (!checkColumn || (columnBitmask == 0)) {
			return _getValue(segmentsEntryRelModelImpl, columnNames, original);
		}
	...
	private static Object[] _getValue(
		...
		for (int i = 0; i < arguments.length; i++) {
			...
			if (original) {
				arguments[i] = segmentsEntryRelModelImpl.getColumnOriginalValue(
					columnName);
			}
			else {
