...
/**
 * The arguments resolver class for retrieving value from HtmlPreviewEntry.
 *
...
@Component(
	immediate = true,
	service = {
		HtmlPreviewEntryModelArgumentsResolver.class, ArgumentsResolver.class
	}
)
public class HtmlPreviewEntryModelArgumentsResolver
	...
	public Object[] getArguments(
		...
		if (!checkColumn || (columnBitmask == 0)) {
			return _getValue(htmlPreviewEntryModelImpl, columnNames, original);
		}
	...
	private static Object[] _getValue(
		...
		for (int i = 0; i < arguments.length; i++) {
			...
			if (original) {
				arguments[i] = htmlPreviewEntryModelImpl.getColumnOriginalValue(
					columnName);
			}
			else {
