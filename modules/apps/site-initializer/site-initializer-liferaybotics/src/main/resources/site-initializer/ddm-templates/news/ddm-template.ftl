...
<style>
	...
	.bg-custom-img
	{
		background-image:url('${Photo.getData()}');
...
<div class="bg-custom-img custom-jumbotron jumbotron jumbotron-fluid">
	<div class="container overlay-top">
		<h1 class="display-4 text-white">${Title.getData()}</h1>

		<p class="lead text-white">${Header.getData()}</p>
	</div>

	<i class="back fa-4x fa-arrow-circle-left fas" onclick="window.history.back()"></i>
