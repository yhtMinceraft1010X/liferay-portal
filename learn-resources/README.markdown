...
The user can get help by clickling on the link (circled above) generated by a `liferay-learn:message` tag.

A link requires two parts:
...
Specifying resources in a separate file facilitates linking to localized resources and updating labels and URLs.

**Note:** If a `liferay-learn:message` tag references a missing JSON file or unspecified resource entry, the tag simply doesn't render--it's fail-safe.
